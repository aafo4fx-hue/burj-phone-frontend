      ...
      >
        ...
        <div className="px-6 py-6 space-y-4 text-center">
          <p className="text-gray-600 text-sm leading-8">
            ...
            <span className="font-extrabold text-gray-800">سنتواصل معك على واتساب</span><br />
            لتأكيد الطلب وترتيب التوصيل.<br />
            <span className="text-[#8543C0] font-bold">الدفع عند الاستلام</span>
          </p>

          {/* HIDDEN – زرار واتساب (محفوظ للرجوع)
          <a href={`https://wa.me/${wa}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3.5 rounded-2xl bg-[#25D366] text-white font-bold text-sm shadow-[0_8px_24px_rgba(37,211,102,0.3)] hover:shadow-[0_12px_32px_rgba(37,211,102,0.4)] transition-shadow">
...
export default function CustomerForm({ total: _total, itemCount: _itemCount, initialData, installmentMonths: _installmentMonths, onSubmit: _onSubmit }: CustomerFormProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-4">
      {showPopup && <OrderPopup whatsapp={whatsapp} onClose={() => setShowPopup(false)} />}
      ...
      {/* HIDDEN – طريقة الدفع والتقسيط (محفوظ للرجوع)
