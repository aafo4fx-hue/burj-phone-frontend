...
export function StepVariant({
  ...
  const activeOpt =
    storageOpts.find((o) => o.storage === storage) ?? storageOpts[0];
  const price =
    activeOpt?.salePrice ?? activeOpt?.originalPrice ?? product.price;
        ...
        {img && (
          <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-lg bg-white border border-gray-100 flex items-center justify-center shrink-0 overflow-hidden">
            <Image
              ...
              width={56}
              height={56}
              className="object-contain p-1 scale-[2]"
            />
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="font-bold text-xs sm:text-sm text-gray-800 leading-snug truncate">
            {product.name}
          </p>
            ...
            {variants.map((v) => (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    color === v.color
                      ? "0 0 0 2px #fff, 0 0 0 3.5px #7A2FCC"
            ...
            {storageOpts.map((opt) => {
              ...
              return (
                <button
                  ...
                  style={{
                    borderColor: active ? "#7A2FCC" : "#E5E7EB",
                    background: active ? "#f3eafc" : "#fff",
                    color: active ? "#5B1FA0" : "#374151",
                  }}
                >
                  ...
                  <span
                    ...
                    style={{ color: active ? "#7A2FCC" : "#9CA3AF" }}
                  >
                    {fmt(p)} ر.س
                  </span>
                </button>
